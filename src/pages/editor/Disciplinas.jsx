...
export default function Disciplinas() {
  ...
  const [filasPorPaginaActividades, setFilasPorPaginaActividades] = useState(10);
  ...
  // ── Co-profesores de un horario (ej. dos profes de Natación juntos) ──
  const handleCoprofesores = async (h) => {
    let actuales = [];
    try {
      actuales = await horariosApi.getCoprofesores(h.id_horario);
    } catch (err) {
      Swal.fire("Error", err.message, "error");
      return;
    }
    const idsActuales = new Set(actuales.map((p) => String(p.id_profesor)));
    const opciones = profesores.filter((p) => Number(p.id_profesor) !== Number(h.id_profesor));
    ...
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="mb-2 small text-muted">
            ${h.dia_h} ${h.hora_h?.slice(0, 5)} — titular: ${h.profesor_nombre || "sin asignar"}.
            Marcá quién más da esta misma clase. Cada uno cobra su sueldo completo, de forma independiente.
          </p>
          ${opciones.map((p) => `
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="${p.id_profesor}" id="swal-coprof-${p.id_profesor}" ${idsActuales.has(String(p.id_profesor)) ? "checked" : ""} />
              <label class="form-check-label" for="swal-coprof-${p.id_profesor}">${p.nomap_p}</label>
            </div>
          `).join("") || '<p class="text-muted">No hay otros profesores para asignar.</p>'}
        </div>
      `,
      showCancelButton: true,
      ...
      preConfirm: () => opciones
        .map((p) => p.id_profesor)
        .filter((id) => document.getElementById(`swal-coprof-${id}`)?.checked),
    });
    if (!formValues) return;

    try {
      await horariosApi.setCoprofesores(h.id_horario, formValues);
      Swal.fire("¡Listo!", "Co-profesores actualizados", "success");
      await recargarHorariosActividad(actividadHorario);
    } catch (err) {
      Swal.fire("Error", err.message, "error");
    }
        ...
        <div className="table-responsive">
          <table className="table table-hover align-middle table-disciplinas">
            <thead className="table-light">
              <tr>
                <th>Nombre</th>
                <th style={{ maxWidth: 260 }}>Descripción</th>
                <th>Activo</th>
                <th className="text-center" style={{ minWidth: 150 }}>Opciones</th>
              </tr>
            </thead>
            <tbody>
              {disciplinasPagina.map((d) => (
                ...
                >
                  <td>{d.nombre_d}</td>
                  <td className="text-truncate" style={{ maxWidth: 260 }} title={d.descripcion_d}>
                    {d.descripcion_d}
                  </td>
                  <td>
                  ...
                  <td className="text-center" onClick={(e) => e.stopPropagation()}>
                    <div className="d-flex justify-content-center flex-wrap gap-1">
                      <button className="btn btn-sm btn-outline-secondary" title="Imágenes carrusel" onClick={() => abrirImagenes(d)}>
                        <i className="ri-image-2-line"></i>
                      </button>
                      <button className="btn btn-sm btn-outline-secondary" title="Editar" onClick={() => abrirEditar(d)}>
      ...
      {disciplinaSeleccionada && (
        <>
          ...
          {loadingActividades ? (
            ...
            const inicioAct = (paginaActividades - 1) * filasPorPaginaActividades;
            const actividadesPagina = actividades.slice(inicioAct, inicioAct + filasPorPaginaActividades);
            return (
              <>
                <div className="table-responsive">
                  <table className="table table-hover align-middle">
                    ...
                    <tbody>
                      {actividadesPagina.length === 0 ? (
                        <tr><td colSpan="5" className="text-center text-muted">No hay actividades cargadas</td></tr>
                      ) : (
                        actividadesPagina.map((a) => (
                          <tr key={`${a.id_actividad}-${a.id_profesor || 'sin'}`}>
                            ...
                            <td>
                              {Number(a.cupo_maximo) > 0 ? (
                                <span className={`badge ${Number(a.cupo_ocupado) >= Number(a.cupo_maximo) ? "bg-danger" : "bg-secondary"}`}>
                                  {a.cupo_ocupado}/{a.cupo_maximo}
                                </span>
                              ) : (
                            ...
                            <td className="text-center">
                              <div className="d-flex justify-content-center flex-wrap gap-1">
                                <button className="btn btn-sm btn-outline-secondary" title="Horarios" onClick={() => abrirHorarios(a)}>
                                  <i className="ri-time-line"></i>
                                </button>
                                <button className="btn btn-sm btn-outline-secondary" title="Editar" onClick={() => abrirEditarActividad(a)}>
                                  <i className="ri-pencil-fill"></i>
                                </button>
                                <button
                ...
                <PaginacionTabla
                  paginaActual={paginaActividades}
                  setPaginaActual={setPaginaActividades}
                  filasPorPagina={filasPorPaginaActividades}
                  setFilasPorPagina={setFilasPorPaginaActividades}
                      ...
                      return (
                        <div className="row mb-3 align-items-center" key={h.id_horario}>
                          ...
                          <div className="col-4">
                            ...
                            {h.coprofesores_nombres && (
                              <small className="text-muted d-block mt-1">+ {h.coprofesores_nombres}</small>
                            )}
                          </div>
                          {/* CUPO MÁXIMO */}
                          <div className="col-2">
                          ...
                          <div className="col-auto" style={{ width: 50 }}>
                            <button className="btn btn-sm btn-outline-secondary"
                              title="Co-profesores (dan la misma clase juntos)"
                              onClick={() => handleCoprofesores(h)}>
